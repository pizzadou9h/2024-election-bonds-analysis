...
for _, row_company in df_company.iterrows():
    purchase_date = row_company['Date']
    purchaser_name = row_company['Purchaser Name']
    purchase_amount = row_company['Denomination']
    
    # Find the matching redemption within 14 days
    mask = (df_party['Date'] >= purchase_date) & (df_party['Date'] <= purchase_date + timedelta(days=14))
    matching_redemptions = df_party.loc[mask]
    
    # Iterate over each matching redemption
    for _, row_party in matching_redemptions.iterrows():
        ...
        if redemption_amount == purchase_amount:
            ...
            df_party.drop(row_party.name, inplace=True)  # Remove the matched redemption from df_party
...
df_matched.to_csv('matched_transactions.csv', index=False)
...
#%%
# how many lines in e-bonds_company.csv
import pandas as pd
df_company = pd.read_csv('e-bonds_company.csv')
print(len(df_company))

# aggregate denomination by purchaser name and print
df_company['Denomination'].sum()

#%%
# how many lines in e-bonds_party.csv
df_party = pd.read_csv('e-bonds_party.csv')
print(len(df_party))
# aggregate denomination by political party and print
df_party['Denomination'].sum()

#%%
# how many lines in matched_transactions.csv
df_matched = pd.read_csv('matched_transactions.csv')
print(len(df_matched))

# aggregate denomination
df_matched['Amount'].sum()

#%%
# how many lines in matched_transactions_rev.csv
df_matched_rev = pd.read_csv('matched_transactions_rev.csv')
print(len(df_matched_rev))

# aggregate denomination
df_matched_rev['Amount'].sum()
